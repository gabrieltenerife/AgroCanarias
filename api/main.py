...
async def lifespan(app: FastAPI):
    from Integrations.Mpcs import cerrar_mcp, obtener_herramientas
    from Agent.Agent import Agente

    mcp_tools, _, _ = await obtener_herramientas()
    app.state.agente, app.state.conn = await Agente(tools=mcp_tools)
    print("Agente y MCP inicializados")
    yield
    await cerrar_mcp()
    await app.state.conn.close()
    print("Agente y MCP cerrados")
...
async def chat_stream(request: ChatRequest, agente=Depends(get_agent)):
    """Envia mensaje al agente y devuelve la respuesta en streaming."""

    async def generate_response():
        try:
            config = {"configurable": {"thread_id": request.thread_id}}
            async for event in agente.astream(
